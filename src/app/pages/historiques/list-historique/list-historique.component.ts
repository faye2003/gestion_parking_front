...
export class HistoriqueComponent implements OnInit {
  ...
  openModal(modalname: any, item: any, action: any) {
    if (item !== null) {
      ...
      this.editForm.patchValue(this.historique);
    } else {
      this.editForm.reset();
    }
  ...
  if (this.actionModal == "add") {
    if (this.editForm.valid) {
      ...
      this.historiqueService.createHistorique(formData)
      .subscribe({
        next: (response: any) => {
    ...
    this.isSubmitted = true;
    this.historiqueService.updateHistorique(this.historique.id, this.editForm.value)
      .subscribe({
        next: (response: any) => {
          if (!response?.status) {
  ...
  deleteHistorique() {
    this.historiqueService.deleteHistorique(this.historique.id).subscribe(
      (response: any) => {
